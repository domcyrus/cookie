...
func ReadSetCookies(h http.Header) []*Cookie {
...
// silently dropped.
func SetCookie(h http.Header, cookie *Cookie) http.Header {
...
func (c *Cookie) String() string {
	...
	if validCookieExpires(c.Expires) {
		...
		b.Write(c.Expires.UTC().AppendFormat(buf[:0], http.TimeFormat))
	}
	if c.MaxAge > 0 {
...
func ReadCookies(h http.Header, filter string) []*Cookie {
	lines := h["Cookie"]
	if len(lines) == 0 {
		return []*Cookie{}
	}
